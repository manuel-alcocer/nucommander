...
void Ui::put_menu() {
    for (auto& item : menu_items) {
        panes.at((size_t)Panes::BOTTOM)->print(item->name);
    }
    ...
    switch (menu) {
        case Menus::FILE:
            if (this->menu != nullptr){
                this->menu = std::make_unique<Pane>();
                this->menu->set_dim(Dim::HEIGHT, 10);
                this->menu->set_dim(Dim::WIDTH, 10);
                this->menu->set_dim(Dim::X, 10);
                this->menu->set_dim(Dim::Y, 10);
                this->menu->set_color_pairs(3, 4);
                this->menu->set_border_enabled(true);
                this->menu->set_layout_spacing(0);
                this->menu->set_cursor(1, 1);
                this->menu->resize();
                this->menu->draw_border();
                this->menu->panel_top();
                this->menu->refresh();
            }
            break;
...
void Ui::initialize_panes() {
    for (int i = 0; i < 4; i++)
        panes.push_back(std::make_unique<Pane>());
...
void Ui::initialize_menus(){
    for (int i = 0; i < (static_cast<int>(Menus::COUNT)); i++)
        menus.push_back(std::make_unique<Pane>());
}
